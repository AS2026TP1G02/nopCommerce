...
| João Varela | `[~]` Plugin-side structured logs carrying `OrderGuid` + `messageId` + `externalRequestId` (publisher + callback paths carry the 3 IDs; full error-path coverage to verify) | `.../OmnichannelCore/` (cross-cutting) |
