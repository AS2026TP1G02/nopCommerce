...
public partial class PriceListService : IPriceListService
{
    ...
    /// </returns>
    public virtual async Task<IPagedList<PriceList>> GetAllPriceListsAsync(int[] customerRoleIds = null, int[] customerIds = null, bool? isActive = null,
        int pageIndex = 0, int pageSize = int.MaxValue)
    {
        var customers = await _priceListRepository.GetAllPagedAsync(query =>
        {
            ...
            if (hasCustomerRoleFilter || hasCustomerFilter)
            {
                ...
                if (hasCustomerRoleFilter && hasCustomerFilter)
                {
                    query = roleFilteredQuery.Union(customerFilteredQuery);
                }
                else if (hasCustomerRoleFilter)
                {
                    query = roleFilteredQuery;
                }
                else
                {
                    query = customerFilteredQuery;
                }
    ...
    /// Gets a price list
    /// </summary>
    /// <param name="priceList">Price list identifier</param>
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains the price list
    /// </returns>
    public virtual async Task<PriceList> GetPriceListByIdAsync(int priceList)
    {
        return await _priceListRepository.GetByIdAsync(priceList);
    }
    ...
    {
        return await _priceListRepository.GetByIdsAsync(priceListIds, cache => default, false);
    }
        ...
        int pageIndex = 0, int pageSize = int.MaxValue)
    {
        ...
        var query = from pc in _priceListItemRepository.Table
        ...
        int pageIndex = 0, int pageSize = int.MaxValue)
    {
        ...
        var query = from plc in _priceListCustomerRepository.Table
