...
public partial class PriceListController : BaseAdminController
{
    ...
    //    var filterLevelValues = await _priceListService.GetPriceListsByIdsAsync(selectedIds.ToArray());

    //    await _priceListService.DeletePriceListsAsync(filterLevelValues);

    //    //activity log
    //    var activityLogFormat = await _localizationService.GetResourceAsync("ActivityLog.DeletePriceList");
    //    await _customerActivityService.InsertActivitiesAsync("DeletePriceList", filterLevelValues, filterLevelValue => string.Format(activityLogFormat, filterLevelValue.Id));
